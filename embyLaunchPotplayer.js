...
(function () {
    ...
    async function embyPot() {
        ...
        window.addEventListener('message', async (event) => {
            if (event.data.type === 'playbackProgress') {
                const { positionTicks } = event.data;
                await updatePlaybackProgress(mediaInfo.itemId, positionTicks);
            }
        });
    }

    async function updatePlaybackProgress(itemId, positionTicks) {
        const userId = ApiClient._serverInfo.UserId;
        const playbackInfo = {
            ItemId: itemId,
            PositionTicks: positionTicks,
            IsPaused: false,
            PlaySessionId: ApiClient._deviceId
        };
        await ApiClient.updatePlaybackProgress(userId, playbackInfo);
        console.log("播放进度已同步到 Emby");
    }
